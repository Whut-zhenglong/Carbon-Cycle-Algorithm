function [carbonfromanimal2decom,fitnessfromanimal2decom] = animaldie(functionname,carboninit,internow,maxinter,LB,UB,Ps,Dim,F_num,fitnessinit)
T = (maxinter-internow)/maxinter;
a = 1.5;
b = 1.5;
f = (a/(1-maxinter))*internow+a-a/(1-maxinter);
Beta = b*(f-f*internow/maxinter)^2;

for i=1:size(carboninit,1)
    Num = ceil(3*rand());
    temp(i,:) = (1-(1+T)*rand()).*carboninit(Num,:);
...
carbonfromanimal2decom = Sortmatrix(1:Ps,1:end-1);
fitnessfromanimal2decom = Sortmatrix(1:Ps,end);
end